class ResponsesController < ApplicationController
 ...
 def index
  ...
  def create
    ...
    respond_to do |format|
      if ( params[:save_and_next] == nil && params[:save_only] == nil && params[:save_on_preview] == "true")
        ...
        if @response.save
          @response.plan.touch
          format.html { redirect_to details_plan_path(@plan, requirement_id: @next_requirement_id), notice: t('.success_message')}
          format.json { render action: 'show', status: :created, location: @response }
        else
          format.html { redirect_to details_plan_path(@plan, requirement_id: @next_requirement_id) }
          format.json { render json: @response.errors, status: :unprocessable_entity }
        ...
        if @response.save
          @response.plan.touch
          format.html { redirect_to details_plan_path(@plan, requirement_id: @requirement_id), notice: t('.success_message')}
          format.json { render action: 'show', status: :created, location: @response }
        else
          format.html { redirect_to details_plan_path(@plan, requirement_id: @requirement_id) }
          format.json { render json: @response.errors, status: :unprocessable_entity }
  ...
  def update
    ...
    flash[:error] = t('.concurrency_error')
